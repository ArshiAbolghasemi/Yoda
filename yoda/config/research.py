...
class PanelConfig:
    ...
    realized_vol_windows: tuple[int, ...] = (5, 20, 60)
...
class NewsLLMConfig:
    ...
    embed_dim: int = 16  # rationale embedding is projected to this width
    timeout: float = 60.0
    seed: int = 0
...
@dataclass(frozen=True)
class NewsConfig:
    backend: str = "llm_agent"  # llm_agent | encoder | none
    cache: str = "processed/news_features.parquet"
    encoder_sentiment_model: str = "ProsusAI/finbert"
    encoder_embed_model: str = "sentence-transformers/all-MiniLM-L6-v2"
    llm: NewsLLMConfig = field(default_factory=NewsLLMConfig)

    def __post_init__(self) -> None:
        if self.backend not in {"llm_agent", "encoder", "none"}:
...
def build_research_config(settings: Dynaconf) -> ResearchConfig:
    ...
    llm = {str(k).lower(): v for k, v in dict(news.pop("llm", {}) or {}).items()}
    llm.setdefault("base_url", getenv("OPENAI_BASE_URL", ""))
    llm.setdefault("api_key", getenv("OPENAI_API_KEY", ""))
    news["llm"] = NewsLLMConfig(**llm)
